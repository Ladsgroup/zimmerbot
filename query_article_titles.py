...
#		]
def query_articles(search_item, language):
	data = get_data(search_item, language)
	result = []
...
def get_article_names_from_query(search_item, language):
	article_dictionaries = query_articles(search_item, language)
	result = []
...
# 	Output: None
def get_search_suggestion(search_item, language):
	data = get_data(search_item, language)
...
def get_data(search_item, language):
	with urllib.request.urlopen(build_url(search_item, language)) as url:
...
query_articles("tapas", "Spanish")
get_article_names_from_query("Albert Einstein", "Icelandic")
get_search_suggestion("asdf Einstein!", "English")
